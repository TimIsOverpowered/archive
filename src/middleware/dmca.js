      ...
      for (let i = 0; i < paths.length; i++) {
        const data = {
          ...
          title:
            type === 'vod'
              ? `${config.channel} ${
                  vod_data.platform.charAt(0).toUpperCase() + vod_data.platform.slice(1)
      ...
      await youtube.upload(
        {
          ...
          title:
            type === 'vod'
              ? `${config.channel} ${
                  vod_data.platform.charAt(0).toUpperCase() + vod_data.platform.slice(1)
                } VOD - ${dayjs(vod_data.createdAt).tz(config.timezone).format('MMMM DD YYYY').toUpperCase()}`
              : `${config.channel} ${
                  vod_data.platform.charAt(0).toUpperCase() + vod_data.platform.slice(1)
                } Live VOD - ${dayjs(vod_data.createdAt).tz(config.timezone).format('MMMM DD YYYY').toUpperCase()}`,
          public:
