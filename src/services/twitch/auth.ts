...
/**
 * Manages Twitch App Access Tokens with in-memory caching, Redis persistence,
 * and distributed locking to prevent concurrent token refreshes.
 */
export class TwitchTokenManager {
  ...
  async getAppAccessToken(): Promise<string> {
    ...
    if (this.tokenState != null && this.tokenState.expiresAt > now + 60 * 60 * 1000) {
  ...
  private async tryLoadFromRedis(): Promise<{ token: string; expiresAt: number } | null> {
  ...
  private async saveToRedis(token: string, expiresAt: number): Promise<void> {
    const redis = RedisService.getActiveClient();
    if (!redis) return;
  ...
  private async acquireLock(): Promise<boolean> {
    ...
    try {
      this.lockValue = `${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
      const result = await redis.set(Twitch.REDIS_LOCK_KEY, this.lockValue, 'EX', Twitch.LOCK_TTL, 'NX');
      return result === 'OK';
    } catch (err) {
      const { message } = extractErrorDetails(err);
  ...
  private async refreshToken(): Promise<string> {
    ...
    try {
      ...
      this.tokenState = { token: access_token, expiresAt };
      await this.saveToRedis(access_token, expiresAt);
      ...
      if (trackFailure(Token.TWITCH_FAILURE_KEY, Token.MAX_FAILURES)) {
        sendDiscordAlert(`🚨 Twitch token refresh failed after ${Token.MAX_FAILURES} attempts: ${message}`).catch(
          (e) => {
...
export async function getAppAccessToken(): Promise<string> {
  return twitchTokenManager.getAppAccessToken();
}
