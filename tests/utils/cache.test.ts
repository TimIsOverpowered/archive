...
describe('withCache', () => {
  ...
  describe('when Redis is disabled', () => {
    it('should call fetcher and return result without caching', async () => {
      ...
      const result1 = await withCache('test-key', 60, fetcher);
      const result2 = await withCache('test-key', 60, fetcher);
    ...
    it('should call fetcher on every request when cache is disabled', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        await withCache('repeat-key', 60, fetcher);
      }
    ...
    it('should return different results from repeated fetcher calls', async () => {
      ...
      const result1 = await withCache('counter-key', 60, fetcher);
      const result2 = await withCache('counter-key', 60, fetcher);
  ...
  describe('error handling', () => {
    it('should return fetcher result when fetcher succeeds', async () => {
      ...
      const result = await withCache('success-key', 60, fetcher);
    ...
    it('should propagate fetcher errors', async () => {
      ...
      try {
        await withCache('error-key', 60, fetcher);
  ...
  describe('fetcher behavior', () => {
    it('should pass through fetcher results unchanged', async () => {
      ...
      for (const testCase of testCases) {
        ...
        const result = await withCache(`type-test-${JSON.stringify(testCase)}`, 60, fetcher);
    ...
    it('should handle empty string result', async () => {
      const fetcher = async () => '';
      const result = await withCache('empty-key', 60, fetcher);
      assert.strictEqual(result, '');
    });

    it('should handle zero result', async () => {
      const fetcher = async () => 0;
      const result = await withCache('zero-key', 60, fetcher);
      assert.strictEqual(result, 0);
    });

    it('should handle false result', async () => {
      const fetcher = async () => false;
      const result = await withCache('false-key', 60, fetcher);
      assert.strictEqual(result, false);
    });

    it('should handle empty object result', async () => {
      const fetcher = async () => ({});
      const result = await withCache('empty-obj-key', 60, fetcher);
      assert.deepStrictEqual(result, {});
    });

    it('should handle empty array result', async () => {
      const fetcher = async () => [];
      const result = await withCache('empty-arr-key', 60, fetcher);
      assert.deepStrictEqual(result, []);
    });
  });
});
...
describe('withStaleWhileRevalidate', () => {
  ...
  describe('when Redis is disabled', () => {
    it('should call fetcher and return result without caching', async () => {
      ...
      const result = await withStaleWhileRevalidate('swr-key', 60, 30, fetcher);
    ...
    it('should call fetcher on every request when cache is disabled', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        await withStaleWhileRevalidate('swr-repeat-key', 60, 30, fetcher);
      }
  ...
  describe('error handling', () => {
    it('should return fetcher result on success', async () => {
      ...
      const result = await withStaleWhileRevalidate('swr-success-key', 60, 30, fetcher);
    ...
    it('should handle fetcher errors gracefully', async () => {
      ...
      try {
        await withStaleWhileRevalidate('swr-fetcher-error', 60, 30, fetcher);
        assert.fail('Should have thrown');
      } catch (error) {
        assert.ok(error instanceof Error);
    ...
    it('should propagate error type', async () => {
      ...
      try {
        await withStaleWhileRevalidate('swr-custom-error', 60, 30, fetcher);
        assert.fail('Should have thrown');
      } catch (error) {
  ...
  describe('fetcher behavior', () => {
    it('should pass through various data types', async () => {
      ...
      for (const testCase of testCases) {
        const fetcher = async () => testCase;
        const result = await withStaleWhileRevalidate(`swr-type-${JSON.stringify(testCase)}`, 60, 30, fetcher);
        assert.deepStrictEqual(result, testCase);
      }
    });
    ...
    it('should handle null result', async () => {
      ...
      const result = await withStaleWhileRevalidate('swr-null-key', 60, 30, fetcher);
      assert.strictEqual(result, null);
    });

    it('should handle undefined result', async () => {
      const fetcher = async () => undefined;
      const result = await withStaleWhileRevalidate('swr-undefined-key', 60, 30, fetcher);
    ...
    it('should handle complex nested objects', async () => {
      ...
      const result = await withStaleWhileRevalidate('swr-complex-key', 60, 30, fetcher);
